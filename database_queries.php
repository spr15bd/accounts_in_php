		...
		if (isset($items['data'])) {
            if (isset($items['data']['supplierName'])) {
                if ($items['data']['supplierName']==='all') {
                    ...
                    $query = "SELECT i.id, i.supplier, i.date, i.number, i.description, i.office, i.overhead, i.amount, p.paidid, p.amount FROM invoices i ";
                ...
                foreach ($items['data']['paid'] as $paidInvoiceID) {
                    $query = "INSERT INTO paid (id) VALUES ('".$paidInvoiceID."');";
                    $sql_query = $conn->query($query);
                    if ($sql_query) {
